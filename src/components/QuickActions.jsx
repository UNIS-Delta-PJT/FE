...
export default function QuickActions({ onScan, onDirectInput }) {
  return (
    <div className="grid grid-cols-2 gap-3" style={{ width: 353, padding: '5px' }}>
      {/* 직접 입력 */}
      <button onClick={onDirectInput} className="flex flex-col items-center justify-center gap-2 rounded-3xl bg-[#1CD1A1]/100 border border-[#1CD1A1]/100 shadow-md active:scale-95 transition-transform" style={{ paddingTop: '5px', paddingBottom: '5px' }}>
        <div className="w-10 h-10 rounded-xl bg-[#1CD1A1]/20 flex items-center justify-center">
          <PlusCircle size={25} color="#FFFFFF" strokeWidth={2} />
        </div>
